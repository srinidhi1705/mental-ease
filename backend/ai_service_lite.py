...
class AIService:
    def __init__(self):
        logger.info("Using lightweight AI service (no ML dependencies)")
        
        # Simple keyword-based emotion detection
        self.emotion_keywords = {
            'joy': ['happy', 'joy', 'excited', 'great', 'awesome', 'wonderful', 'amazing', 'fantastic', 'love', 'perfect', 'excellent'],
            'sadness': ['sad', 'down', 'depressed', 'upset', 'crying', 'hurt', 'disappointed', 'lonely', 'miserable', 'devastated'],
            'anger': ['angry', 'mad', 'furious', 'annoyed', 'frustrated', 'irritated', 'rage', 'hate', 'pissed', 'outraged'],
            'fear': ['anxious', 'worried', 'nervous', 'stressed', 'panic', 'fear', 'scared', 'terrified', 'afraid', 'overwhelmed'],
            'surprise': ['surprised', 'shocked', 'amazed', 'astonished', 'unexpected', 'wow', 'incredible', 'unbelievable'],
            'disgust': ['disgusted', 'gross', 'awful', 'terrible', 'horrible', 'nasty', 'revolting', 'sick'],
    ...
    def get_dominant_emotion(self, text: str) -> str:
        ...
        # Only return emotion if there's a clear match
        ...
        for crisis_category, keywords in self.crisis_keywords.items():
            for keyword in keywords:
                if keyword in message_lower:
                    ...
                    if crisis_category in ['suicide', 'self_harm', 'violence','kill','die','knife']:
                        severity = 'high'
    ...
    def generate_response(self, message: str, detected_emotion: str = None, user_name: str = None) -> str:
        ...
        if detected_emotion in self.emotion_responses:
            return random.choice(self.emotion_responses[detected_emotion])
    ...
    def analyze_conversation_sentiment(self, messages: List[str]) -> Dict:
        ...
        for message in messages:
            emotion = self.get_dominant_emotion(message)
    ...
    def _get_how_to_response(self, message_lower: str) -> Optional[str]:
        ...
        keyword_mapping = {
            ...
            'deal_with_anxiety': ['anxiety', 'anxious', 'worry', 'nervous', 'panic'],
